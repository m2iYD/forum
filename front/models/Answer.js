export class Answer {
  ...
  static async findAll() {
    try {
      const response = await fetch("http://localhost:8001/api/answers");
  ...
  static async findById(id) {
    try {
      const response = await fetch(`http://localhost:8001/api/answers/${id}`);
  ...
  static async findByQuestionId(question_id) {
    try {
      const response = await fetch(
        `http://localhost:8001/api/questions/${question_id}/answers`
      );
      ...
      console.error(
        `Error fetching answers for question ID ${question_id}:`,
        error
      );
      return [];
    }
  ...
  static async fakeFindAll() {
    return [
      ...
      new Answer(
        3,
        "Event loop works asynchronously.",
        7,
        1,
        103,
        3,
